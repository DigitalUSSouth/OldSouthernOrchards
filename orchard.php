...
if (isset($_GET['orcName']))
{
	...
	$query = $con2->prepare("SELECT filename FROM orc_images WHERE Name = ?");
	$query->bind_param('s', $orcName);
}
$query->execute();
$query->store_result();
$query->bind_result($orcName);
echo '<div id="content">';
echo '<div style="padding-top: 20px; padding-bottom: 0;"><a href="index.php">&lt; Back to Home</a></div>';
echo '<div style="line-height: 0; padding-bottom: 15px; margin-bottom: 15px;"><h1>'.$_GET['orcName'].'</h1></div>';
while ($query->fetch())
	{
		echo '<img src="images/'.$orcName.'" style="margin-left:auto; margin-right:auto;" />';
	}
echo '</div>';
?>
</body>
